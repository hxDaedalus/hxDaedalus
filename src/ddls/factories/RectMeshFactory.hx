...
class RectMeshFactory
{
	...
	public static function buildRectangle(width:Float, height:Float):Mesh
	{
		...
		*/
		...
		var vTL:Vertex = new Vertex();
		var vTR:Vertex = new Vertex();
		var vBR:Vertex = new Vertex();
		var vBL:Vertex = new Vertex();
		
		var eTL_TR:Edge = new Edge();
		var eTR_TL:Edge = new Edge();
		var eTR_BR:Edge = new Edge();
		var eBR_TR:Edge = new Edge();
		var eBR_BL:Edge = new Edge();
		var eBL_BR:Edge = new Edge();
		var eBL_TL:Edge = new Edge();
		var eTL_BL:Edge = new Edge();
		var eTR_BL:Edge = new Edge();
		var eBL_TR:Edge = new Edge();
		var eTL_BR:Edge = new Edge();
		var eBR_TL:Edge = new Edge();
		
		var fTL_BL_TR:Face = new Face();
		var fTR_BL_BR:Face = new Face();
		var fTL_BR_BL:Face = new Face();
		var fTL_TR_BR:Face = new Face();
		...
		vTL.fromConstraintSegments.push(segTop);
		vTL.fromConstraintSegments.push(segLeft);
		vTR.fromConstraintSegments.push(segTop);
		vTR.fromConstraintSegments.push(segRight);
		vBR.fromConstraintSegments.push(segRight);
		vBR.fromConstraintSegments.push(segBot);
		vBL.fromConstraintSegments.push(segBot);
		vBL.fromConstraintSegments.push(segLeft);
		...
		for (f in [segTop, segRight, segBot, segLeft])
			boundShape.segments.push(f);
		
		for (f in [vTL, vTR, vBR, vBL])
			mesh.__vertices.push(f);
			
		for (f in [eTL_TR, eTR_TL, eTR_BR, eBR_TR, eBR_BL, eBL_BR, eBL_TL, eTL_BL, eTR_BL, eBL_TR, eTL_BR, eBR_TL])
			mesh.__edges.push(f);
		
		for (f in [fTL_BL_TR, fTR_BL_BR, fTL_BR_BL, fTL_TR_BR])
			mesh.__faces.push(f);
		...
		for (f in [0, 0, width, 0])
			securityRect.push(f);
		
		for (f in [width, 0, width, height])
			securityRect.push(f);
			
		for (f in [width, height, 0, height])
			securityRect.push(f);
		
		for (f in [0, height, 0, 0])
			securityRect.push(f);
