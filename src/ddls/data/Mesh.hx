...
import ddls.iterators.FromVertexToOutgoingEdges;
...
class Mesh
{
	...
	private var _vertices:Array<Vertex> = null;
	private var _edges:Array<Edge> = null;
	private var _faces:Array<Face> = null;
	...
	public var __vertices(get, null):Array<Vertex>;
	private function get___vertices():Array<Vertex>
	{
		return _vertices;
	}
	
	public var __edges(get, null):Array<Edge>;
	private function get___edges():Array<Edge>
	{
		return _edges;
	}

	public var __faces(get, null):Array<Face>;
	private function get___faces():Array<Face>
	{
		return _faces;
	}
	
	public var __constraintShapes(get, null):Array<ConstraintShape>;
	...
	{
		if (object.constraintShape != null)
			deleteObject(object);
	...
	public function insertConstraintSegment(x1:Float, y1:Float, x2:Float, y2:Float):ConstraintSegment
	{
		...
		*/
		...
		var vertexDown:Vertex = insertVertex(newX1, newY1);
		if (vertexDown == null)
			return null;
		var vertexUp:Vertex = insertVertex(newX2, newY2);
		if (vertexUp == null)
			return null;
		...
		var segment:ConstraintSegment = new ConstraintSegment();
		...
		while (true)
		{
			...
			if (Std.is(currObjet, Vertex))
			{
				currVertex = cast currObjet;
				//trace("case vertex");
			...
			else if (Std.is(currObjet, Edge))
			{
				currEdge = cast currObjet;
				//trace("case edge");
	...
	public function insertVertex(x:Float, y:Float):Vertex
	{
		...
		var inObject:Dynamic = Geom2D.locatePosition(x, y, this);
		var inVertex:Vertex;
		var inEdge:Edge;
		var inFace:Face;
		var newVertex:Vertex = null;
		if (Std.is(inObject, Vertex))
		{
			inVertex = cast inObject;
			//trace("inVertex", inVertex.id);
			newVertex = inVertex;
		}
		else if (Std.is(inObject, Edge))
		{
			inEdge = cast inObject;
			//trace("inEdge", inEdge);
			newVertex = splitEdge(inEdge, x, y);
		}
		else if (Std.is(inObject, Face))
		{
			inFace = cast inObject;
			//trace("inFace");
	...
	public function flipEdge(edge:Edge):Edge
	{
		...
		var eBot_Top:Edge = edge;
		var eTop_Bot:Edge = edge.oppositeEdge;
		var eLeft_Right:Edge = new Edge();
		var eRight_Left:Edge = new Edge();
		var eTop_Left:Edge = eBot_Top.nextLeftEdge;
		var eLeft_Bot:Edge = eTop_Left.nextLeftEdge;
		var eBot_Right:Edge = eTop_Bot.nextLeftEdge;
		var eRight_Top:Edge = eBot_Right.nextLeftEdge;
		
		var vBot:Vertex = eBot_Top.originVertex;
		var vTop:Vertex = eTop_Bot.originVertex;
		var vLeft:Vertex = eLeft_Bot.originVertex;
		var vRight:Vertex = eRight_Top.originVertex;
		
		var fLeft:Face = eBot_Top.leftFace;
		var fRight:Face = eTop_Bot.leftFace;
		var fBot:Face = new Face();
		var fTop:Face = new Face();
	...
	public function splitEdge(edge:Edge, x:Float, y:Float):Vertex
	{
		...
		var eLeft_Right:Edge = edge;
		var eRight_Left:Edge = eLeft_Right.oppositeEdge;
		var eRight_Top:Edge = eLeft_Right.nextLeftEdge;
		var eTop_Left:Edge = eRight_Top.nextLeftEdge;
		var eLeft_Bot:Edge = eRight_Left.nextLeftEdge;
		var eBot_Right:Edge = eLeft_Bot.nextLeftEdge;
		
		var vTop:Vertex = eTop_Left.originVertex;
		var vLeft:Vertex = eLeft_Right.originVertex;
		var vBot:Vertex = eBot_Right.originVertex;
		var vRight:Vertex = eRight_Left.originVertex;
		
		var fTop:Face = eLeft_Right.leftFace;
		var fBot:Face = eRight_Left.leftFace;
		...
		// create new objects
		var vCenter:Vertex = new Vertex();
		
		var eTop_Center:Edge = new Edge();
		var eCenter_Top:Edge = new Edge();
		var eBot_Center:Edge = new Edge();
		var eCenter_Bot:Edge = new Edge();
		
		var eLeft_Center:Edge = new Edge();
		var eCenter_Left:Edge = new Edge();
		var eRight_Center:Edge = new Edge();
		var eCenter_Right:Edge = new Edge();
		
		var fTopLeft:Face = new Face();
		var fBotLeft:Face = new Face();
		var fBotRight:Face = new Face();
		var fTopRight:Face = new Face();
		...
		{
			var fromSegments:Array<ConstraintSegment> = eLeft_Right.fromConstraintSegments;
	...
	public function splitFace(face:Face, x:Float, y:Float):Vertex
	{
		...
		var eTop_Left:Edge = face.edge;
		var eLeft_Right:Edge = eTop_Left.nextLeftEdge;
		var eRight_Top:Edge = eLeft_Right.nextLeftEdge;
	...
	public function deleteVertex(vertex:Vertex):Bool
	{
		...
		var edge:Edge;
		var outgoingEdges:Array<Edge> = new Array<Edge>();
		
		var boundA:Array<Edge> = null;
		var boundB:Array<Edge> = null;
		...
		var bound:Array<Edge> = new Array<Edge>();
		if (freeOfConstraint)
		{
	...
	private function triangulate(bound:Array<Edge>, isReal:Bool):Void
	{
		...
		else // if more than 3 edges, we process recursively:
		{
			...
			/*for (i in 0...bound.length ; i++)
			{
				//trace("  - edge", i, ":", bound[i].originVertex.id, "->", bound[i].destinationVertex.id);
			}*/
				
			var baseEdge:Edge = bound[0];
			var vertexA:Vertex = baseEdge.originVertex;
			var vertexB:Vertex = baseEdge.destinationVertex;
			var vertexC:Vertex;
			var vertexCheck:Vertex;
			var circumcenter:Point2D = new Point2D();
			...
			boundM = new Array<Edge>();
			if (index == 2) {
				boundM.push(baseEdge);
				boundM.push(bound[1]);
				boundM.push(edgeAopp);
			} else if (index == (bound.length - 1)) {
				boundM.push(baseEdge);
				boundM.push(edgeB);
				boundM.push(bound[index]);
			} else {
				boundM.push(baseEdge);
				boundM.push(edgeB);
				boundM.push(edgeAopp);
