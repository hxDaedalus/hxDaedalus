...
class BresenHamPixels {
    ...
    public static function setPixelAA( pixels: IPixel, x: Int, y: Int, i:Float, col: Int, alpha: Int ) {
        i = 1-i/alpha;
        pixels.setPixelColorAndAlpha( x, y, col, 255*i );
    }
    ...
    public inline static function plotEllipse(  pixels:     IPixels
                                            ...
                                            ,   col: Int,   alpha: Int ){
        var x = -a;
        var y = 0;                              /* II. quadrant from bottom left to top right */
        var e2 = b*b;
        ...
        do {
            ...
            pixels.setPixelColorAndAlpha( xm + x, ym - y );                 /* III. Quadrant */
            pixels.setPixelColorAndAlpha( xm - x, ym - y );                 /*  IV. Quadrant */
            e2 = 2*err;
    ...
    public inline static function plotCircle( pixels:   IPixels
        ...
        var ix: Int = -r;
        var y: Int = 0;
        var err: Int = 2 - 2*r;                /* bottom left to top right */

        do {
            pixels.setPixelColorAndAlpha( xm - x, ym + y, col, alpha );  /*   I. Quadrant +x +y */
            pixels.setPixelColorAndAlpha( xm - y, ym - x, col, alpha );  /*  II. Quadrant -x +y */
            pixels.setPixelColorAndAlpha( xm + x, ym - y, col, alpha );  /* III. Quadrant -x -y */
            pixels.setPixelColorAndAlpha( xm + y, ym + x, col, alpha );  /*  IV. Quadrant +x -y */
            r = err;
            if( r <= y ){
    ...
    public inline static function plotQuadBezierSeg(    pixels:     IPixels
                                                    ,   x0: Int,    y0: Int
                                                    ,   x1: Int,    y1: Int
                                                    ,   x2: Int,    y2: Int
                                                    ,   col: Int,   alpha: Int ){
    ...
    public static function plotQuadRationalBezierSeg(   pixels:     IPixel
                                                    ,   x0: Int,    y0: Int
                                                    ,   x1: Int,    y1: Int
                                                    ,   x2: Int,    y2: Int
                                                    ,   w: Float
        ...
        if(cur != 0.0 && w > 0.0) {                  /* no straight line */
            ...
            if( w < 0.5 && ( dy > xy || dx < xy ) ) {    /* flat ellipse, algorithm fails */
                ...
                plotQuadRationalBezierSeg( pixels, x0, y0, dx, dy, sx, sy, cur, col, alpha );/* plot separately */
                dx = Math.floor( ( w*x1 + x2 )*xy + 0.5 );
                dy = Math.floor( ( y1*w + y2 )*xy + 0.5 );
                plotQuadRationalBezierSeg( pixels, sx, sy, dx, dy, x2, y2, cur, col, alpha );
                return;
            }
            err = dx + dy - xy;                                                 /* error 1.step */
            do {
                pixels.setPixelColorAndAlpha( pixels, x0, y0, col, alpha );     /* plot curve */
                if (x0 == x2 && y0 == y2) return;                               /* last pixel -> curve finished */
                x1 = 2*err > dy;
                y1 = 2*( err + yy) < -dy;                                       /* save value for test of x step */
                if(2*err < dx || y1 ) {                                         /* y step */
                    y0 += sy;
                    dy += xy;
                    err += dx += xx;
                }
    ...
    public inline static function plotRotatedEllipseRect( pixels:    IPixels
                                                        , x0: Int,   y0: Int
                                                        , x1: Int,   y1: Int
                                                        , zd: Float
                                                        , col: Int,  alpha: Int ){
    ...
    public inline static function plotCubicBezierSeg( pixels:  IPixels
                                                    , x0:      Int,    y0:     Int
                                                    , x1:      Float,  y1:     Float
                                                    , x2:      Float,  y2:     Float
                                                    , x3:      Int,    y3:     Int
        ...
        var yc: Float = -Math.abse( y0 + y1 - y2 - y3 );
    ...
    public inline static function plotCubicBezier(  pixels:     IPixels
        ...
        for( i = 0...(n + 1) ){                 /* plot each segment separately */
    ...
    public inline static function plotLineAA( pixels:   Pixels
                                            , x0: Int,  y0: Int
                                            , x1: Int,  y1: Int
                                            , col: Int, alpha: Int ){
        var sx: Int = ( x0 < x1 )? 1 : -1;
        ...
        var num: Float = 0xffff7fl/Math.sqrt( err )
        ...
        while( true ){
            ...
            setPixelAA( pixels, x0, y0, Math.abs( err - dx + dy ) >> 16, col, alpha );
            e2 = err;
            x2 = x0;
            if( 2*e2 >= -dx ){                                            /* x step */
                if( x0 == x1 ) break;
                if( (e2 + dy) < 0xff0000l ) setPixelAA( pixels, x0, y0 + sy, ( e2 + dy ) >> 16, col, alpha );
                err -= dy;
                x0 += sx;
            }
            if( 2*e2 <= dy ) {                                             /* y step */
                if( y0 == y1 ) break;
                if( dx - e2 < 0xff0000l ) setPixelAA( pixels, x2 + sx, y0, ( dx - e2 ) >> 16, col, alpha );
                err += dx;
                y0 += sy;
                count++;
            }
    ...
    public inline static function plotQuadBezierSegAA(  pixels:     IPixels
                                                    ,   x0: Int,    y0: Int
                                                    ,   x1: Int,    y1: Int
                                                    ,   x2: Int,    y2: Int
                                                    ,   col: Int,   alpha: Int ){
    ...
    public inline static function plotQuadRationalBezierSegAA(  pixels:     IPixels
      ...
      if( w < 0.5 && dy > dx ){              /* flat ellipse, algorithm fails */
         ...
         dx = Math.floor( ( w*x1 + x0 )*xy + 0.5 );
         dy = Math.floor( ( y1*w + y0 )*xy + 0.5 );
         plotQuadRationalBezierSegAA( pixels, x0, y0, dx, dy, sx, sy, cur, col, alpha ); /* plot apart */
         dx = Math.floor( ( w*x1 + x2 )*xy + 0.5);
         dy = Math.floor( ( y1*w + y2 )*xy + 0.5);
         return plotQuadRationalBezierSegAA( pixels, sx, sy, dx, dy, x2, y2, cur, col, alpha );
      }
      err = dx + dy - xy;                                       /* error 1st step */
      do {                                                      /* pixel loop */
         cur = Math.min( dx - xy, xy - dy );
...
public inline static function plotCubicBezierSegAA( pixels:     IPixels
   ...
   if( xa == 0 && ya == 0 ) {                              /* quadratic Bezier */
      sx = Math.floor( ( 3*x1 - x0 + 1 )/2 );
      sy = Math.floor( ( 3*y1 - y0 + 1 )/2 );   /* new midpoint */
      return plotQuadBezierSegAA( pixels, x0, y0, sx, sy, x3, y3, col, alpha );
   }
   x1 = ( x1 - x0 )*( x1 - x0 ) + ( y1 - y0 )*( y1 - y0 ) + 1;                    /* line lengths */
   x2 = ( x2 - x3 )*( x2 - x3 ) + ( y2 - y3 )*( y2 - y3 ) + 1;
   do {                                                /* loop over both ends */
...
// HOW!!!
      ...
      while( x0 != x3 && y0 != y3 ) {
         ...
         if( 2*fx <= f ) {                           /* y+ anti-aliasing pixel */
            if( px < ed ) setPixelAA( pixels, x0, y2 + sy, 255*px/ed, col, alpha );      /* plot curve */
            x0 += sx; fx += f;                                      /* x step */
         }
      }
      ...
      yb = -yb;
        ...
        plotLineAA( pixels, x0, y0, x3, y3, col, alpha );     /* remaining part in case of cusp or crunode */
    }
                                                ...
                                                ,   col: Int, alpha: Int ){
    {
        ...
        while( true ) {                                    /* pixel loop */
            setPixelColor( pixels, x0, y0, Math.max( 0, 255*( Math.abs( err - dx + dy )/ed - wd + 1 ) ), col, alpha );
            e2 = err;
            x2 = x0;
            if( 2*e2 >= -dx ){
                //////!!!!!!                                            /* x step */
                e2 += dy;
                y2 = y0;
                for( e2 < ed*wd && (y1 != y2 || dx > dy) ){
                    setPixelColor( pixels, x0, y2 += sy, Math.max(0,255*(Math.abs(e2)/ed-wd+1)), col, alpha );
                    e2 += dx;
                }
                if( x0 == x1 ) break;
                e2 = err;
            ...
            if( 2*e2 <= dy ){                                             /* y step */
                ...
                while( e2 < ed*wd && (x1 != x2 || dx < dy) ){
                    setPixelColor( pixels, x2 += sx, y0, Math.max(0,255*(abs(e2)/ed-wd+1)), col, alpha );
                    e2 += dy;
                }
                if( y0 == y1 ) break;
                err += dx;
    ...
    public inline static function plotQuadSpline( pixels: IPixels
        ...
        var M_MAX: Int = 6;
        var mi: Float = 1
        var m[ M_MAX ];                    /* diagonal constants of matrix */
        var i: Int;
        var x0: Int
        var y0: Int
        var x1: Int
        ...
        for( i in 0...n-2 ){ //= n-2; i > 0; i--) {                           /* back substitution */
            ...
            plotQuadBezier( pixels, ( x0 + x1 )/2, ( y0 + y1 )/2, x1, y1, x2, y2, col, alpha );
            x2 = ( x0 + x1 )/2;
            x1 = x0;
            y2 = ( y0 + y1 )/2;
            y1 = y0;
    ...
    public inline static function plotCubicSpline(  pixels:         IPixels
        ...
        plotCubicBezier( pixels, x3, y3, ( x2 + x4 )/2, ( y2 + y4 )/2, x4, y4, x4, y4, col, alpha );

        if( n-3 < M_MAX ) mi = m[ n - 3 ];
        x1 = Math.floor( ( x[ n - 2 ] - 2*x2 )*mi + 0.5 );
        y1 = Math.floor( ( y[ n - 2 ] - 2*y2 )*mi + 0.5 );
        var ii: Int;
        for( i is 0...n-3 ) {                           /* back substitution */
            ii = n-3 - i;
            if( ii <= M_MAX ) mi = m[ ii - 1 ];
        ...
        plotCubicBezier(    pixels, x4, y4
                        ...
                        ,   Math.floor( ( x1 + 2*x2 )/3 + 0.5), Math.floor( ( y1 + 2*y2 )/3 + 0.5 )
                        ,   x3, y3, col, alpha );
        plotCubicBezier( pixels, x0, y0, x0, y0, ( x0 + x1 )/2, ( y0 + y1 )/2, x4, y4, col, alpha );
    }
